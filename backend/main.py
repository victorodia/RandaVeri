...
DEFAULT_SUBSCRIPTION_PRICE = float(os.getenv("DEFAULT_SUBSCRIPTION_PRICE", 50000.0))  # Default yearly license fee
...
def debug_db(db: Session = Depends(get_db)):
    ...
    return {
        ...
        "admin_roles": db.query(AdminRole).count(),
        "activity_logs": db.query(ActivityLog).count(),
        "org_names": [o.name for o in db.query(Organisation).limit(5).all()],
        "usernames": [u.username for u in db.query(User).limit(5).all()]
    }
